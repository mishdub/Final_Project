        ...
        for dex, polygon in enumerate(sorted_items):
            ...
            if pol2.within(pol1):
                ...
                if dex >= 1:
                    ...
                    l_f2, l_t2 = check3
                    u_f2, u_t2 = check4
                    the_angle1 = self.calculate_angle_in_degrees(l_t, l_t2)
                    the_angle2 = self.calculate_angle_in_degrees(l_t, u_t2)
                    the_angle1 = (the_angle1 + 0.01) % 360
                    the_angle2 = (the_angle2 + 0.01) % 360
                    left_point = polygon.left_point

                    flag_1, d1, d2, d3, d4, d5, d6, extended_poly1, right_li, left_li = self.placement2(
                        the_angle1,
                        polygon.coordinates,
                        previous_polygon)
                    print(flag_1)
                    flag_2, d1, d2, d3, d4, d5, d6, extended_poly2, right_li, left_li = self.placement2(
                        the_angle2,
                        polygon.coordinates,
                        previous_polygon)
                    print(flag_2)
                    if dex >= 1:
                        ...
                        copied.set_coordinates(list(extended_poly2.exterior.coords))
                    ...
                    new_angle = None
                    if flag_1:
                        result1 = self.intersection_of_lines4(vertical_line, horizontal_line, the_angle1, u_f2,
                                                              convex_region)

                        if isinstance(result1, tuple):
                            new_angle, new_point = result1
                            polygon.move_from_to2(l_t, new_point)
                        else:
                            stan = True
                    else:
                        result2 = self.intersection_of_lines4(vertical_line, horizontal_line, the_angle2, u_f2,
                                                              convex_region)
                        if isinstance(result2, tuple):
                            new_angle, new_point = result2
                            polygon.move_from_to2(l_t, new_point)
                        else:
                            stan = True
                    if stan:
                        polygon.move_from_to2(l_t, middle_point)
                        new_angle = self.calculate_angle_in_degrees(u_f, l_t)
