...
class Algo26:
    ...
    def temp_func(self, polygon, p_of_convex, p_of_middle ,convex_region):
        ...
        proj_p = self.project_point_to_linestring_edge_new(new_line, p_of_convex)
    ...
    def temp_func2(self, polygon, p_of_convex, p_of_middle):
        ed_li = self.find_edges_with_vertex(polygon, p_of_convex)
        angle1 = self.calculate_angle_in_degrees(ed_li[0], p_of_convex)
        angle2 = self.calculate_angle_in_degrees(ed_li[1], p_of_convex)
        angle1_180 = (angle1 + 180) % 360
        ...
        line = LineString([closest_point, p_of_convex])
        pol = Polygon(polygon.coordinates)
        i_p = self.find_intersection_point(polygon.coordinates, [closest_point, p_of_convex], closest_point)
        return i_p
                            ...
                            for point in points:
                                ...
                                if flag:
                                    ...
                                    if j_index == 0:
                                        ...
                                        p_of_tran = self.temp_func2(copiedp, a, point)


                                        rec_of_poly_copied.move_from_to2(point, p_of_tran)
                                        polygon.move_from_to2(point, p_of_tran)
                                        points = self.check_if_line_cross(previous_polygon, rec_of_poly_copied)
                                        if dex >= 1000:
                                            line3 = []
                                            line3.append([a, point])
