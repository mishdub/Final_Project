...
class Algo9:
    ...
    def move_poly_LineString(self, polygon, angle, line_st):
        ...
        list_of_lines = []
        line_st = self.round_and_convert_to_int(line_st)
        ...
        for point_line in line_st.coords:
            ...
            if p is not None:
                ...
                if distance < min_distance:
                    ...
                    print("checked2")
                    list_of_lines.append([(xl, yl), (x1, y1)])
    ...
    def place_poly(self, original_polygon, extended_poly, convex_region, angle):
        ...
        list_of_points = []
        list_of_l = []
        print("convex regin",list(convex_exterior.coords))
        ...
        if extended_poly.intersects(convex_exterior):
            ...
            if intersection.is_empty:
                ...
                if intersection.geom_type == "LineString":
                    ...
                    f_p1, t_p1, list_of_points1, list_of_l1, dis1 = self.move_poly_MultiLineString2(original_polygon,
                                                                                                    angle,
                                                                                                    intersection.geoms)

                    f_p2, t_p2, list_of_points2, list_of_l2, dis2 = self.move_poly_MultiLineString(original_polygon,
                                                                                                   angle,
                                                                                                   intersection.geoms)
    ...
    def placement(self, angle, middle_polygon, convex_polygon):
        ...
        cx, cy = self.round_and_convert_to_int_single(center)
        vx, vy = (
            math.cos(math.radians(angle)), math.sin(math.radians(angle)))
        ...
        if not (filled_polygon.intersects(pol)):
            flag = True
        return flag, (px1, py1), p1, (px2, py2), p2, (cx, cy), (x1, y1), filled_polygon,list(left_line.coords),list(right_line.coords)
    ...
    def extend_pol(self, angle, convex_region, polygon):
        ...
        vx, vy = self.round_and_convert_to_int_single((vx, vy))
    ...
    def calculate_endpoint_from_direction(self, x1, y1, dx, dy, length):
        ...
        x2, y2 = self.round_and_convert_to_int_single((x2, y2))
    ...
    def round_and_convert_to_int(self,coordinates, decimal_places=0):
        """
        Rounds each coordinate in the list to the specified number of decimal places
        and converts them to integers.

        Parameters:
        - coordinates (list of tuples): List of (x, y) coordinates.
        - decimal_places (int): Number of decimal places to round to (default is 0).

        Returns:
        - List of rounded and converted integer coordinates.
        """
        rounded_coordinates = [(round(x, decimal_places), round(y, decimal_places)) for x, y in coordinates]
        int_coordinates = [(int(x), int(y)) for x, y in rounded_coordinates]
        return int_coordinates

    def round_and_convert_to_int_single(self,coord, decimal_places=0):
        """
        Rounds each value in the coordinate tuple to the specified number of decimal places
        and converts them to integers.

        Parameters:
        - coord (tuple): A tuple containing two values (x, y).
        - decimal_places (int): Number of decimal places to round to (default is 0).

        Returns:
        - Tuple of rounded and converted integer values.
        """
        rounded_values = (round(coord[0], decimal_places), round(coord[1], decimal_places))
        int_values = (int(rounded_values[0]), int(rounded_values[1]))
        return int_values
    ...
    def plot(self):
        ...
        x,y = self.round_and_convert_to_int_single((x,y))
        ...
        for dex, polygon in enumerate(sorted_items):
            ...
            polygon.set_coordinates(self.round_and_convert_to_int(polygon.coordinates))
            ...
            if pol2.within(pol1):
                print(dex)
                if dex == 0:
                    extended_polygon = self.extend_pol_for_first_time(angle, polygon,(x,y))
                    list_of_extended_polygon = list(extended_polygon.exterior.coords)
                    extended_polygon = Polygon(self.round_and_convert_to_int(list_of_extended_polygon))
                    ...
                    list_of_new_region = self.round_and_convert_to_int(list_of_new_region)
                ...
                if dex >= 1:
                    ...
                    flag = False
                    while not flag:
                        flag, d1, d2, d3, d4, d5, d6, extended_polygon, left_line, right_line = self.placement(angle, polygon.coordinates,
                                                                                        previous_polygon)
                        ...
                        if flag:
                            ...
                            f_p, t_p, list_of_lines, list_of_points = self.place_poly(polygon, extended_polygon,
                                                                                      convex_region, angle)
                            lefti = LineString(left_line)
                            righti = LineString(right_line)
                            pl = Polygon(convex_region).exterior
                            print("what intresection is it ",lefti.intersection(pl))
                            print("what intresection is it ",righti.intersection(pl))
                            ...
                            if dex >= 2:
                                copied.set_coordinates(list_of_extended_polygon)
