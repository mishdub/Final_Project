            ...
            if pol2.within(pol1):
                ...
                if dex >= 1:
                    ...
                    if not Polygon(polygon.coordinates).within(pol1):
                        ...
                        cal_angle = self.calculate_angle_in_degrees(u_f2, l_t2)
                        cal_angle2 = self.calculate_angle_in_degrees(u_f2, u_t2)


                        result1 = self.intersection_of_lines4(vertical_line, horizontal_line, cal_angle, u_f2,
                                                              convex_region)
                        result2 = self.intersection_of_lines4(vertical_line, horizontal_line, cal_angle2, u_f2,
                                                              convex_region)

                        if isinstance(result1, tuple) and isinstance(result2, tuple):
                            final_angle1, final_point1 = result1
                            final_angle2, final_point2 = result2
                            pal1 = polygon.move_from_to2_value(u_f2, final_point1)
                            pal2 = polygon.move_from_to2_value(u_f2, final_point2)
                            for j_index in range(2):
                                if j_index == 0:
                                    final_angle1 = (final_angle1 + 0.01) % 360
                                    ...
                                    copied2.set_coordinates(pal1)

                                    print("first", flag)

                                    f_p, t_p, list_of_lines, list_of_points = self.place_poly(copied2, extended_poly,
                                                                                              convex_region,
                                                                                              final_angle1,
                                                                                              right_li,
                                                                                              left_li)
                                    pal1 = copied2.move_from_to2_value(f_p, t_p)

                                    final_f_p1 = f_p
                                    final_t_p1 = t_p
                                    if flag:
                                        polygon.move_from_to2(l_t, final_point1)
                                        polygon.move_from_to2(final_f_p1, final_t_p1)
                                else:
                                    final_angle2 = (final_angle2 + 0.01) % 360
                                    flag, d1, d2, d3, d4, d5, d6, extended_poly, right_li, left_li = self.placement(
                                        final_angle2,
                                        pal2,
                                        previous_polygon)
                                    copied2 = copy.deepcopy(polygon)
                                    copied2.set_coordinates(pal2)
                                    ...
                                    f_p, t_p, list_of_lines, list_of_points = self.place_poly(copied2, extended_poly,
                                                                                              ...
                                                                                              right_li,
                                                                                              left_li)
                                    pal2 = copied2.move_from_to2_value(f_p, t_p)
                                    final_f_p2 = f_p
                                    final_t_p2 = t_p
                                    if flag:
                                        polygon.move_from_to2(l_t, final_point2)
                                        polygon.move_from_to2(final_f_p2, final_t_p2)
                                    print("sec", flag)
                        ...
                        l_f2, l_t2 = check3
                        u_f2, u_t2 = check4
                        cal_angle = self.calculate_angle_in_degrees(l_t, l_t2)
                        the_angle, the_point = self.intersection_of_lines4(vertical_line, horizontal_line, cal_angle, l_t,
                                                              convex_region)
                        flag, d1, d2, d3, d4, d5, d6, extended_poly, right_li, left_li = self.placement(
                            the_angle,
                            polygon.coordinates,
