...
class Algo8:
    ...
    def calculate_next_position(self, previous_position,angular_offset):

        """
        Calculate the next position of a polygon based on the previous position, a fixed radius,
        and an angular offset.

        Parameters:
        - previous_position: Tuple (x, y) representing the coordinates of the previous polygon's center.
        - radius: The fixed radius between the centers of adjacent polygons.
        - angular_offset: The angular offset in degrees.

        Returns:
        - Tuple (x, y) representing the coordinates of the next polygon's center.
        """
        convex_center = self.container_instance.calculate_centroid()
        radius = self.calculate_radius(previous_position, convex_center)
        x_previous, y_previous = previous_position
        x_next = x_previous + radius * math.cos(math.radians(angular_offset))
        y_next = y_previous + radius * math.sin(math.radians(angular_offset))

        angle = self.calculate_angle((x_next, y_next), convex_center)
        return angle

    def calculate_radius(self, prev_center, convex_center):
        """
        Calculate the radius based on the distance between the middle point of the previous polygon
        and the center of the convex region.

        Parameters:
        - prev_center: Tuple (x, y) representing the middle point of the previous polygon.
        - convex_center: Tuple (x, y) representing the center of the convex region.

        Returns:
        - The calculated radius.
        """
        distance = math.sqrt((convex_center[0] - prev_center[0]) ** 2 + (convex_center[1] - prev_center[1]) ** 2)
        return distance
