...
class Algo28:
    ...
    def project_point_to_convex_edge2(self,convex_region, point):
        # Create a Shapely Polygon from the convex region
        polygon = Polygon(convex_region)

        # Create a Shapely Point for the given point
        input_point = Point(point)

        # Find the nearest point on the convex hull edge
        nearest_point = nearest_points(input_point, polygon.exterior)[0]

        # Check if the nearest point is inside the convex hull
        if polygon.contains(nearest_point):
            return nearest_point.x, nearest_point.y
        else:
            # If not inside, project onto the line segment connecting the two closest vertices
            line = LineString([point, nearest_point])
            projected_point = line.interpolate(line.project(input_point))
            return projected_point.x, projected_point.y
    ...
    def func_to_find_best_pos(self, tri_index,triangle_list,edges_list,convex_region_original,convex_region,a):
        ...
        return tri_index, p, deter_angle_point,proj_p

    def scale_convex_region1(self,coords, new_length):
        # Create a Shapely Polygon object from the input coordinates
        polygon = Polygon(coords)

        # Get the centroid of the polygon
        centroid = Point(polygon.centroid)

        # Translate the polygon so that the centroid is at the origin
        translated_polygon = Polygon([(x - centroid.x, y - centroid.y) for x, y in polygon.exterior.coords])

        # Scale the polygon by the ratio of the new length to the original distance from centroid to boundary
        original_distance_to_boundary = max(
            translated_polygon.exterior.coords.xy[0])  # Assuming convex region is in the x-axis
        scale_factor = new_length / original_distance_to_boundary
        scaled_polygon = Polygon([(x * scale_factor, y * scale_factor) for x, y in translated_polygon.exterior.coords])

        # Translate the scaled polygon back to its original position
        final_polygon = Polygon([(x + centroid.x, y + centroid.y) for x, y in scaled_polygon.exterior.coords])

        # Extract the coordinates of the scaled polygon as a list of tuples
        scaled_coords = list(final_polygon.exterior.coords)

        return scaled_coords

    def create_new_convex_region_with_angle(self, coords, angle, new_intersection_point):
        # Create a Shapely Polygon object from the input coordinates
        original_polygon = Polygon(coords)

        # Get the centroid of the original polygon
        original_centroid = Point(original_polygon.centroid)

        # Calculate the length of the translation vector based on the angle
        translation_length = original_centroid.distance(Point(new_intersection_point))

        # Calculate the translation vector components
        translation_vector = Point(math.cos(math.radians(angle)) * translation_length,
                                   math.sin(math.radians(angle)) * translation_length)

        # Calculate the translation to keep the centroid in the same position
        translation_to_centroid = Point(original_centroid.x - (original_centroid.x + translation_vector.x),
                                        original_centroid.y - (original_centroid.y + translation_vector.y))

        # Combine both translations
        total_translation = Point(translation_vector.x + translation_to_centroid.x,
                                  translation_vector.y + translation_to_centroid.y)

        # Translate the original polygon to match the new intersection point and angle while keeping the centroid fixed
        translated_polygon = translate(original_polygon, xoff=total_translation.x, yoff=total_translation.y)

        # Extract the coordinates of the translated polygon as a list of tuples
        new_coords = list(translated_polygon.exterior.coords)
        ...
        for dex, polygon in enumerate(sorted_items):
            ...
            if pol2.within(pol1):
                if dex == 0:
                    ...
                    polygon.left_point_temp = the_point
                ...
                if dex >= 1:
                    ...
                    while not flag_temp:
                        ...
                        p_of_projection = None
                        polygon_var2 = copy.deepcopy(polygon)

                        while not flag000:
                            flag_v = False
                            copiedpol = copy.deepcopy(previous_polygon)
                            if previous_polygon.left_intersection_point is not None:
                                flag_v = True
                            ...
                            for j_index in range(2):
                                if j_index == 1:
                                    ...
                                    if flag_v:
                                        print("did it go here1")
                                        vertical_line, horizontal_line = self.create_lines(convex_region)

                                        proj_p = previous_polygon.left_intersection_point
                                        p_of_projection = proj_p
                                        if dex == 1:
                                            p_of_c = previous_polygon.left_point
                                        else:
                                            p_of_c = previous_polygon.p_point
                                        ...
                                        print("did it go here2")

                                        tri_index, p, deter_angle_point, p_of_projection = self.func_to_find_best_pos(tri_index,
                                                                                                     triangle_list,
                                                                                                     edges_list,
                                                                                                     convex_region_original,
                                                                                                     convex_region,
                                    ...
                                    if pol_check2.within(pol1):
                                        ...
                                        sign_yes = True
                                        break
                                points = self.check_if_line_cross(copiedpol, polygon)
                                for point in points:
                                    line = []
                                    poi = []
                                    ...
                                    while True:
                                        temp_a = a
                                        a, b, c = self.check_ep(angle, copiedpol, point)
                                        if a == temp_a and a is not None:
                                            a, b, c = self.check_ep3(angle, copiedpol, point, a)
                                        angle = self.calculate_angle_in_degrees(point, a)
                                        if True:
                                            dime = self.container_instance.calculate_total_dimensions()
                                            xx, yy = point
                                            ...
                                            xxx, yyy = self.calculate_endpoint_from_direction(xx, yy, vx, vy, dime)
                                            l = LineString([point, (xxx, yyy)])
                                        p = Polygon(copiedpol.coordinates)
                                        p = p.buffer(0.1)
                                        if not l.crosses(p):
                                            to_point_temp = a
                                            # angle = angle_before
                                    ...
                                    if dex >= 10000:
                                        ...
                                        if p_of_projection is not None:
                                            poi.append(p_of_projection)
                        ...
                        if sec_flag:
                            ...
                            if not sign_yes:
                                f_p_temp, t_p_temp, list_of_lines_temp, list_of_points_temp = self.place_poly(copied_polygon, extended_polygon,
                                                                                          convex_region_original, angle,
                                                                                          right_line,
                                                                                          left_line)

                                copied_polygon.move_from_to2(f_p_temp, t_p_temp)
                                the_point_temp, sec_point_temp, left_list_temp = self.check_ep(angle, copied_polygon, middle_point)

                                org_pol = Polygon(convex_region_original).exterior

                                if Point(the_point_temp).within(org_pol):
                                    print("point is in")

                                    proj_p = self.project_point_to_convex_edge(convex_region_original, the_point_temp)
                                else:
                                    print("point is out")
                                    proj_p = self.project_point_to_convex_edge1(convex_region_original, the_point_temp)
                                check_point = org_pol.intersection(left_line)
                                print(check_point)
                                polygon.left_intersection_point = (check_point.x, check_point.y)
                                polygon.left_point_temp = the_point_temp
