        ...
        for dex, polygon in enumerate(sorted_items):
            ...
            if pol2.within(pol1):
                ...
                if dex >= 1:
                    ...
                    poly_rec_curr = self.polygon_to_rectangle(polygon.coordinates)
                    a = [list(t) for t in poly_rec_curr]
                    big_p = previous_polygon.extend_polygon(1.1)
                    poly_rec = self.polygon_to_rectangle(big_p)
                    b = [list(t) for t in poly_rec]
                    check = finder.findUpperTangent(a, b)
                    check2 = finder.findLowerTangent(a, b)
                    l_f, l_t = check
                    u_f, u_t = check2
                    copied2 = copy.deepcopy(polygon)
                    copied2.set_coordinates(poly_rec)
                    our_points = self.find_edges_with_vertex(copied2, l_t)
                    copied3 = copy.deepcopy(polygon)
                    copied3.set_coordinates(poly_rec_curr)
                    pal = copied3.move_from_to2_value(u_f, our_points[0])
                    pal2 = copied3.move_from_to2_value(u_f, our_points[1])

                    test1 = Polygon(pal).intersects(Polygon(poly_rec))
                    test2 = Polygon(pal2).intersects(Polygon(poly_rec))

                    if test1 and test2:
                        print("0")
                        copied4 = copy.deepcopy(copied3)
                        copied4.move_from_to2(u_f, l_t)
                        a = [list(t) for t in copied4.coordinates]
                        big_p = previous_polygon.extend_polygon(1)
                        poly_rec = self.polygon_to_rectangle(big_p)
                        b = [list(t) for t in poly_rec]
                        check5 = finder.findUpperTangent(a, b)
                        check6 = finder.findLowerTangent(a, b)
                        l_f3, l_t3 = check5
                        u_f3, u_t3 = check6
                        f_angle = self.calculate_angle_in_degrees(u_f3, l_t3)
                    elif test1:
                        print("1")
                        f_angle = self.calculate_angle_in_degrees(l_t, our_points[0])
                    else:
                        f_angle = self.calculate_angle_in_degrees(l_t, our_points[1])

                    result = self.intersection_of_lines4(vertical_line, horizontal_line, f_angle, l_t,
                                                          convex_region)
                    ...
                    if isinstance(result, tuple):
                        print("first")
                        final_angle, final_point = result
                        copied3.move_from_to2(u_f, final_point)
                        polygon.move_to_rectangle(copied3.coordinates)
                        a = [list(t) for t in polygon.coordinates]
                        check3 = finder.findUpperTangent(a, b)
                        check4 = finder.findLowerTangent(a, b)
                        l_f2, l_t2 = check3
                        u_f2, u_t2 = check4
                        polygon.move_from_to2(u_f2, final_point)
                        final_angle = (final_angle + 0.01) % 360
                        ...
                        final_angle = self.calculate_angle_in_degrees(u_f, l_t)
                        final_angle = (final_angle + 0.01) % 360
                        flag, d1, d2, d3, d4, d5, d6, extended_poly, right_li, left_li = self.placement2(
                            final_angle,
                            polygon.coordinates,
                    ...
                    if dex >= 11:
                        draw_instance = Draw(self.container_instance, another_list, (1, 1), (1, 1), (1, 1), (1, 1),
                                             None,
                                             None,
                                             None, None)
                        draw_instance.plot()
