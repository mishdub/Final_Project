        ...
        for dex, polygon in enumerate(sorted_items):
            ...
            if pol2.within(pol1):
                ...
                if dex >= 1:
                    ...
                    # Initialize variables to store the farthest line segment and its distance
                    farthest_line = None
                    max_distance = 0.0

                    # Iterate over all edges of the convex hull
                    for i in range(len(convex_region)):
                        point_in_convex = Point(convex_region[i])
                        # Create a LineString representing the current edge
                        current_edge = LineString([(x, y), (convex_region[i])])

                        distance_to_centroid = Point(x, y).distance(point_in_convex)

                        # Update the farthest point if the current one is farther
                        if distance_to_centroid > max_distance:
                            max_distance = distance_to_centroid
                            farthest_line = current_edge

                    # Access the coordinates of the farthest line
                    farthest_line_coords = list(farthest_line.coords)

                    # Calculate the angle in radians
                    angle_radians = math.atan2(farthest_line_coords[1][1] - farthest_line_coords[0][1],
                                               farthest_line_coords[1][0] - farthest_line_coords[0][0])

                    # Convert the angle to degrees and apply modulo 360
                    angle_degrees = math.degrees(angle_radians) % 360

                    angle = angle_degrees

                    flag, d1, d2, d3, d4, d5, d6, extended_polygon, right_line, left_line = self.placement(angle,
                                                                                                           polygon.coordinates,
                                                                                                           previous_polygon)
                    ...
                    if dex >= 100:
                        ...
                        #coordinates_of_convex_hull = convex_hull.points
                        #coordinates_of_convex_hull_list = [tuple(coord) for coord in coordinates_of_convex_hull]
                        #print(coordinates_of_convex_hull_list)
                        # Obtain the coordinates of the convex hull
